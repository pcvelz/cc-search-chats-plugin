...
- **Fix shell parse errors with special characters** — quoting `$ARGUMENTS` prevents `)`, `;`, `|` and other shell metacharacters in queries from breaking execution. Flags like `--tail` and `--limit` continue to work as before.
